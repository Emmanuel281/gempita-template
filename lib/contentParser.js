...
export const getListPage = async (filePath, apiUrl = "") => {
  ...
  const pageData = fs.readFileSync(process.cwd() + filePath, "utf-8");
...
export const getSingleData = async (apiUrl) => {
  let contentapi;
  try {
    const response = await fetch(apiUrl, requestOptions);
    const cborDataAsString = await response.arrayBuffer();
...
export const getSingleDataFile = async (apiUrl) => {
  let contentapi;
  try {
    const response = await fetch(apiUrl, requestOptions);
    const cborDataAsString = await response.arrayBuffer();
  ...
  contentapi = fs.readFileSync("./config/cobapagacara.json", "utf-8");
  return contentapi;
};
